...
ssize_t rio_writen(int fd, void *usrbuf, size_t n) 
{
    ...
    while (nleft > 0) {
        ...
        if ((nwritten = write(fd, bufp, nleft)) <= 0) {
            // 被中断的时候，我们就认为一个字节都没有写成功
            if (errno == EINTR)  /* Interrupted by sig handler return */
                nwritten = 0;    /* and call write() again */
            else
                return -1;       /* errno set by write() */    // 函数调用出错，直接返回
        }
        nleft -= nwritten;
        bufp += nwritten;
    }
...
ssize_t rio_readlineb(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    ...
    for (n = 1; n < maxlen; n++) { 
        // 刚开始rio缓冲区中有数据的话，可以直接拿到，但是没有的话，调用函数会阻塞。
...
int open_listenfd(char *port) 
{
    ...
    /*
        我的简单思考推测，可以就是内核也是具有一个socket数组，类似于file数组，
        socket数组里面每一个对象都是关联到一个 (address, port) 二元组（对方的信息），对于connect和listen的套接字类型
        获取也是不一样的，socket对象里面获取还关联到一个buffer区域，可以存放数据。
    */
